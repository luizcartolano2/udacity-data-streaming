...
kafka_redis_df = kafka_redis_df.selectExpr("cast(value as string) value")
...
kafka_redis_df.withColumn("value", from_json("value", kafka_redis_schema))\
...
z_set_entries_encoded_streaming_df = spark.sql(
...
z_set_decoded_entries_streaming_df = z_set_entries_encoded_streaming_df.withColumn(
    "customer", unbase64(z_set_entries_encoded_streaming_df.encodedCustomer).cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
z_set_decoded_entries_streaming_df.withColumn("customer", from_json("customer", customer_schema))\
    .select(col('customer.*'))\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING
kafka_events_df = kafka_events_df.selectExpr("cast(value as string) value")
...
kafka_events_df.withColumn("value", from_json("value", kafka_event_schema))\
...
stedi_score_streaming_df = customerRiskStreamingDF.join(
...
stedi_score_streaming_df.selectExpr(
    "cast(customer as string) as key",
    "to_json(struct(*)) as value")\
    .writeStream \
    .format("kafka")\
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "stedi-score-2")\
    .option("checkpointLocation", "/tmp/kafkacheckpoint")\
    .start()\
    .awaitTermination()

print(stedi_score_streaming_df)
