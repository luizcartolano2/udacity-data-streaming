...
# TO-DO: using a select expression on the streaming dataframe, cast the key and the value columns from kafka as strings, and then select them
customersStreamingDF = customersRawStreamingDF.selectExpr(
    "cast(key as string) key", "cast(value as string) value")
...
customerSelectStarDF = spark.sql(
    "select customerName, accountNumber as customerNumber from BankCustomers")
...
# . +-------------+------+--------------------+------------+--------------+
customerWithDepositDF.writeStream.outputMode(
