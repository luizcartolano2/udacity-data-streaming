...
customerStreamingDF = spark.sql(
    "select accountNumber as customerAccountNumber, location as homeLocation, birthDay from Customer where birthDay is not null")
...
relevantCustomerFieldsStreamingDF = customerStreamingDF.select(
    'customerAccountNumber', 'homeLocation', split(customerStreamingDF.birthDay, "-").getItem(0).alias("birthYear"))

customerLocationStreamingDF = spark.sql(
...
# +---------------------+-----------+---------------------+------------+---------+
currentAndHomeLocationStreamingDF.writeStream.outputMode(
